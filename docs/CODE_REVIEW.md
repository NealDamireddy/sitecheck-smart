...
| AI-03 | **CRITICAL** | `src/lib/ai-model.ts` (new), `src/lib/ai-vision.ts`, `api/analyze`, `api/scan-swppp`, `smarts-automation/src/vision/llm.ts`, migration 006 | The pinned model `claude-sonnet-4-20250514` was retired. Every AI feature — photo analysis, text analysis, SWPPP extraction, and the bot's vision layer — returned `404 not_found_error`. The QSP saw "AI photo analysis failed — nothing was saved". The string was copied into six files, so nothing in the build noticed. Reported by Neal after uploading a checkpoint photo. | **fixed** — pinned to `claude-opus-5` in one place (`src/lib/ai-model.ts`); the bot keeps a copy it cannot import across the process boundary. Verified against the live API: old ID → 404, new ID → 200. |
| AI-04 | **HIGH** | `src/app/api/scan-swppp/route.ts` | The SWPPP extractor prefilled the assistant turn with `{` to force JSON. Assistant-turn prefills return **400** on every 4.6-and-later model, so this route could not have worked on any current model even after the ID was corrected. Latent — masked by AI-03 failing first. | **fixed** — prefill removed; the JSON-only constraint already existed in both the system and user turns, and the largest-JSON-object fallback remains. |
| AI-05 | **HIGH** | `src/app/api/analyze/route.ts` | Bare `JSON.parse(textContent.text)` on the model reply. Opus 5 wraps its JSON in a ```` ```json ```` fence despite the prompt forbidding fences, so this route returned 502 on **every** request. Invisible to the suite because Anthropic is mocked and the mocks returned bare JSON. Found by calling the live API with a real BMP photo. | **fixed** — shares `extractJsonBlock()` with `ai-vision.ts` via `src/lib/ai-model.ts`. The fence tolerance does not weaken SEC-07: contents are still Zod-validated, proven by tests that a fence cannot smuggle an out-of-range confidence, an oversized summary, non-JSON, or a compliance status. |
| AI-06 | **HIGH** | `src/lib/ai-vision.ts` (system prompt) | The prompt asked for `"cgpReference": "Relevant CGP 2022 section reference and explanation"`. Opus 5 follows instructions more literally than Sonnet 4 and wrote the explanation, blowing the 500-char Zod cap, so every real photo analysis failed validation → 502 → "nothing was saved". Found only by live end-to-end run. | **fixed** — the prompt now specifies a citation only, under 200 chars, with explicit limits for every field and a note that over-long fields are rejected. **The Zod caps were not raised** — they are a SEC-07 injection defense. `tests/model-pin.test.ts` fails the build if a prompt limit ever crosses its schema cap. |
| AI-07 | MEDIUM | `api/analyze`, `ai-vision.ts`, bot vision | `max_tokens: 1024`. From Opus 5 on, thinking is on by default and `max_tokens` caps thinking **plus** response text. A live call spent 1536 output tokens on one photo — the old budget would have truncated mid-JSON. | **fixed** — budgets centralised in `AI_MAX_TOKENS` (8192 analysis / 16384 extraction); a test rejects any bare numeric `max_tokens` at a call site. |
| AI-08 | LOW | `supabase/migrations/006_mission_telemetry_review.sql:62` | `mission_ai_analyses.model` still defaults to the retired ID. Cosmetic: a DEFAULT only, on a drone table that is not live, and every code path passes the model explicitly. | **open** — migrations are review-gated; folded into the pending migration batch rather than applied unreviewed. |

**How AI-04/05/06/07 were found.** The mocked suite passed throughout — it mocks Anthropic at the module boundary, so it validated the code against *its own idea* of a model reply, not a real one. Each of these four surfaced only by uploading a real BMP photo to the private bucket, signing it, and calling the real `analyzeBmpPhoto()` against the live API. Worth remembering the next time green tests are read as proof the integration works: they prove the contract, never the counterparty.
